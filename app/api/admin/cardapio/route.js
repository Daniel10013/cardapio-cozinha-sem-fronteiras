...
export async function GET() {
  ...
  const { data: categorias, error: erroCat } = await db.from('categorias').select('*').order('ordem').order('id');
  if (erroCat) return Response.json({ erro: erroCat.message }, { status: 500 });

  const { data: pratos, error: erroPratos } = await db.from('pratos').select('*').order('ordem').order('id');
