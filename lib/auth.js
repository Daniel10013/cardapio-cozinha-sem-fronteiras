...
export async function statusBloqueio(ip) {
  const { data } = await supabaseAdmin().from('csf_tentativas_login').select('*').eq('ip', ip).maybeSingle();
  ...
  if (Date.now() >= bloqueadoAte.getTime()) {
    await supabaseAdmin().from('csf_tentativas_login').delete().eq('ip', ip);
...
export async function registrarTentativaFalha(ip) {
  ...
  if (falhas >= MAX_TENTATIVAS) {
    ...
    await supabaseAdmin().from('csf_tentativas_login').upsert({ ip, falhas: 0, bloqueado_ate: bloqueadoAte });
  } else {
    await supabaseAdmin().from('csf_tentativas_login').upsert({ ip, falhas, bloqueado_ate: null });
  }
}
